...
customize_templates() {
    ...
    if [[ -f ".aicontext/aicontext-template.md" ]]; then
        sed -i.bak \
            -e "s/\[PROJECT NAME\]/$PROJECT_NAME/g" \
            -e "s/\[DATE\]/$current_date/g" \
            .aicontext/aicontext-template.md
        mv .aicontext/aicontext-template.md .aicontext/aicontext.md
        rm .aicontext/aicontext-template.md.bak
    fi
...
- **[AI Context](.aicontext/aicontext.md)**: Project knowledge base for AI assistants
...
show_next_steps() {
    ...
    echo "  .aicontext/aicontext.md             AI knowledge base"
